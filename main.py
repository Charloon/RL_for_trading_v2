...
if __name__ == '__main__':
    ################ initialize ##################
    # workflows
    FLAG_LOAD_DATA = False          # flag to load saved preprocessed data
    ...
    FLAG_TRAIN = False               # flag to train
    FLAG_PREDICT = True             # flag to predict 
    FLAG_NEW_MODEL = True
    FLAG_EARLY_STOPPING = False
    # parameters
    TOTAL_TIMESTEPS = 1000000       # number for time steps for each training 
    ...
    default_param = {"policy": "MlpLstmPolicy", "verbose":2, "seed": SEED, "n_steps": N_STEP_LEARNING,
    ...
    if FLAG_PREDICT:
        list_metric_train, list_metric_valid, list_metric_test = parallel_predict(metadata, None, data_valid,
                                                                                   data_test, ALGO, STATS_PATH, "", FLAG_PLOT)
        vizualize_prediction_summary(list_metric_train, list_metric_valid, list_metric_test)

    if FLAG_EARLY_STOPPING:
        n_steps_per_stop = int(float(TOTAL_TIMESTEPS)/N_STOPS)
        df_early_stopping = pd.DataFrame(columns=["steps", "train", "valid", "test"], index = range(N_STOPS))
        #global start_timestep
        for i in range(N_STOPS):
            if i > 0:
                FLAG_NEW_MODEL = False 
            start_timestep = i * n_steps_per_stop
            run_training(metadata, data_train, ALGO, default_param, NUM_CPU, n_steps_per_stop,
                    info_keywords = INFO_KEYWORDS, flag_use_opt_hyperparam = FLAG_USE_OPT_HYPERPARAM,
                    flag_use_opt_env = FLAG_USE_OPT_ENV, stats_path = STATS_PATH, flag_new_model = FLAG_NEW_MODEL,
                    seed_env = SEED+i*NUM_CPU, start_timestep = start_timestep, end_timestep = TOTAL_TIMESTEPS)
            list_metric_train, list_metric_valid, list_metric_test = parallel_predict(metadata, data_train, data_valid,
                                                                                       data_test, ALGO, STATS_PATH,
                                                                                       flag_plot=False)
            # save results to file
            df_early_stopping["steps"].values[i] = (i + 1) * n_steps_per_stop 
            df_early_stopping["train"].values[i] = np.mean(list_metric_train) 
            df_early_stopping["valid"].values[i] = np.mean(list_metric_valid) 
            df_early_stopping["test"].values[i] = np.mean(list_metric_test)
            df_early_stopping.to_csv("df_early_stopping.csv")