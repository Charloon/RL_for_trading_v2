...
def get_metrics(suffix, code, tick):
    """
    ...
    file = open(LOG_DIR+"/"+"df_eval_"+suffix+code+tick+".csv", 'rb')
...
def export_study_to_csv(study, name = 'study_env.pkl', suffix = "env"):
    ...
    with open(name, 'rb') as handle:
        ...
        df_opt.to_csv(LOG_DIR+"/"+df_name)
        print("Study exported to", df_name)

def align_env_with_training(metadata, data, stats_path):
