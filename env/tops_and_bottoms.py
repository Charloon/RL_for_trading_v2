...
def approx_beta(df, tops_bottom_feat, ratio):
    """
    Calculate the coefficient beta used in the reward function. 
    Input:
    - df: dataframe to update
    ...
    for i in range(1, df_temp.shape[0]):
        if df_temp[tops_bottom_feat].values[i] != 0:
            ...
            i_end = j
            if last_value == -1:
                price_end = df_temp["High"].values[i_end]
            elif last_value == 1:
                price_end = df_temp["Low"].values[i_end]
            for k in range(i_start+1, i_end+1):
                # calculate espilon
                #eps = 1e-6*price_start
                # base on price
                price = np.clip(df_temp["Close"].values[k], min(price_start, price_end), max(price_start, price_end))
                # for buying - (1) if increasing
                if last_value == 1 and price <= (price_start + (price_end - price_start)*ratio):#+eps:
                    df_temp[name].values[k] = -last_value
                # for selling - (-1) if decreasing
                elif last_value == -1 and price >= (price_start + (price_end - price_start)*ratio):#-eps:
                    df_temp[name].values[k] = -last_value
                else:
                    df_temp[name].values[k] = 0.

                """# based on days
                # for buying
                if last_value == 1 and k <= (i_start + (i_end - i_start)*ratio):#+eps:
                    df_temp[name].values[k] = -last_value
                # for selling
                elif last_value == -1 and k <= (i_start + (i_end - i_start)*ratio):#-eps:
                    df_temp[name].values[k] = -last_value
                else:
                    df_temp[name].values[k] = 0."""
                #### alpha #### -1 at start and 1 at end
...
def find_tops_and_bottom(df, period=14):
    ...
    # add beta
    df_temp["alpha"] = 0.
    df_temp["beta0"] = approx_beta(df = df_temp, tops_bottom_feat = "tops_bottoms_"+str(period)+"MA", ratio = 0.)
    ...
    df["higherTF_beta"] = approx_beta(df = df, tops_bottom_feat = "higherTF_tops_bottoms_"+str(period)+"MA", ratio = 0.)
    #df["higherTF_beta"] = calculate_beta(df = df, tops_bottom_feat = "higherTF_tops_bottoms_"+str(period)+"MA")
    list_feat.append("alpha")


    ############## to temporarily help the algorithm find the solution ############
    #list_feat.append("higherTF_beta_noise")
    #list_feat.append("beta0_noise")
    # add noise 
    noise_ratio = 1.0
    noise =  np.random.uniform(-1,1,df["higherTF_beta"].shape[0])
    """list_index = np.arange(df["higherTF_beta"].shape[0])
    list_index = np.random.shuffle(list_index)
    list_index = list_index[:int(noise_ratio*len(list_index))]
    df["higherTF_beta_noise"] = copy.deepcopy(df["higherTF_beta"])
    df["higherTF_beta_noise"].values[list_index] = noise[list_index]
    df["beta0_noise"] = copy.deepcopy(df["beta0"])
    df["beta0_noise"].values[list_index] = noise[list_index]"""

    #df["higherTF_beta_noise"] = df["higherTF_beta"]*(1.0 - noise_ratio) + noise_ratio*noise
    #df["beta0_noise"] = df["beta0"]*(1.0 - noise_ratio) + noise_ratio*noise
